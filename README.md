...
├── process.py                      # Original Raw-to-sRGB processing logic
├── process_new.py                  # Updated Raw-to-Linear-RGB-to-sRGB processing logic
├── requirements.txt                # Project dependencies
└── README.md                       # This file
```
* `unprocess.py`: Takes an sRGB image and reverses the ISP pipeline (gamma, tone mapping, color correction, etc.) to produce a 4-channel synthetic Bayer raw image.

* `process_new.py`: Implements the forward ISP steps (demosaicing, white balance, color correction) to convert a raw Bayer image into a linear RGB image.

* `generate_low_light_new_coco.py`: The main executable script. It ties everything together:
  1. Loads an sRGB image.
  2. Uses `unprocess.py` to get the synthetic raw data.
  3. Adds configurable shot and read noise.
  4. Uses `process_new.py` to convert the noisy raw image to linear RGB.
  5. Performs the low-light simulation (illumination separation, darkening, detail preservation).
  6. Applies final gamma correction to produce the final sRGB low-light image.

## Getting Started

### 1. Prerequisites

This project uses TensorFlow 1.x (due to `tf.compat.v1`). You will also need OpenCV and NumPy.
```python
