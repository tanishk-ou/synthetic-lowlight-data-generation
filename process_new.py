# This file contains modifications from its original version.
# The original code was part of the google-research/unprocessing repository, 
# licensed under the Apache License, Version 2.0.
#
# Original Copyright (c) [2019] [Tim Brooks, et al.]
#
...
## ADDITIONS 
def process_to_linear_rgb(bayer_images, red_gains, blue_gains, cam2rgbs):
  """Processes a batch of Bayer RGGB images into linear RGB images."""
  bayer_images.shape.assert_is_compatible_with((None, None, None, 4))
  with tf.name_scope(None, 'process_to_linear_rgb'):
    # White balance.
...
def apply_gamma_compression(images, gamma=2.2):
  """Converts from linear to gamma space."""
  # Clamps to prevent numerical instability of gradients near zero.
  return tf.maximum(images, 1e-8) ** (1.0 / gamma)
...
def process(bayer_images, red_gains, blue_gains, cam2rgbs):
  ...
  linear_rgb = process_to_linear_rgb(bayer_images, red_gains, blue_gains, cam2rgbs)
  return apply_gamma_compression(linear_rgb)
